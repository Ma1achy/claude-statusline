...
function build() {
  ...
  let PERM_GLYPH;
  if (PERM.startsWith("accept"))
    PERM_GLYPH = `${ESC}[38;2;255;176;48m\u26A1${R}`;
  else if (PERM.startsWith("bypass"))
    PERM_GLYPH = `${ESC}[38;2;255;82;129m!!${R}`;
  else
    PERM_GLYPH = `${ESC}[38;2;160;150;255m?${R}`;
  let PET = "";
  if (cfg.pet) {
    let face, col;
    if (COST >= 0.5) {
